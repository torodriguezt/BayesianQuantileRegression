data {
  ...
  array[N] int<lower=1, upper=I> internet; // Índice de horas de estrato vivienda
...
transformed parameters {
  //vector[H] horas_cen;
  //horas_cen = horas_effect - mean(horas_effect); 
...
model {
  ...
  for (i in 1:N) {
    y[i] ~ skew_double_exponential(alpha[cole[i]]  + internet_cen[internet[i]], 2*sigma, tau);
  }
}
